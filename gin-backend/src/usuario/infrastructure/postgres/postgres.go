...
func (r *UsuarioPostgresRepository) FindByEmail(ctx context.Context, email string) (*entities.Usuario, error) {
	return r.FindByEmailOrAlias(ctx, email)
}

func (r *UsuarioPostgresRepository) FindByEmailOrAlias(ctx context.Context, value string) (*entities.Usuario, error) {
	const q = `
		SELECT user_id, nombre, email, password, role_id, alias, telefono, residencia_id, eliminado, created_at, updated_at
		FROM usuario
		WHERE (LOWER(email) = $1 OR LOWER(alias) = $1)
		  AND role_id <> $2
		  AND eliminado = FALSE
	`
	credential := strings.TrimSpace(strings.ToLower(value))
	var u entities.Usuario
	err := r.db.QueryRow(ctx, q, credential, core.CIUDADANO).Scan(
		&u.ID, &u.Nombre, &u.Email, &u.PasswordHash, &u.RolID, &u.Alias, &u.Telefono, &u.ResidenciaID, &u.Eliminado, &u.CreatedAt, &u.UpdatedAt,
	)
	if err != nil {
